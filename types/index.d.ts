...
export type SimpleElementProps<T> = Pick<T, { [K in keyof T]: (<U>() => U extends { [L in K]: T[K]; } ? 1 : 2) extends <U>() => U extends { -readonly [L in K]: T[K]; } ? 1 : 2 ? T[K] extends string | number | boolean | ((...args: any[]) => any) | null | undefined ? K : never : never; }[keyof T]>;
...
export type ElementAttributes<T extends Element> = T extends HTMLElement ? DataAttributes : T extends SVGElement ? SVGAttributes : never;
export type ElementProps<T extends Element, Shared = SharedElementProps<T>, Simple = SimpleElementProps<T>, Attributes = ElementAttributes<T>> = { [K in keyof Shared | keyof Simple | keyof Attributes]?: K extends keyof Shared ? Shared[K] : K extends keyof Simple ? Simple[K] | null : K extends keyof Attributes ? Attributes[K] : never; };
